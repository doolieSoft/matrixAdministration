...
public class DatabaseHelper {
    ...
    private void createDatabase() {
        try {
            ...
            statement.executeUpdate("CREATE TABLE RATIO_T (I_AGENT_ID INTEGER, I_CI_ID INTEGER, " + RATIO_FIELD + " INTEGER, UNIQUE(I_AGENT_ID, I_CI_ID))");
            statement.executeUpdate("CREATE TABLE AGENT_T (I_ID INTEGER PRIMARY KEY, ST_AGENT TEXT UNIQUE)");
            statement.executeUpdate("CREATE TABLE CI_T (I_ID INTEGER PRIMARY KEY, ST_CI TEXT UNIQUE)");
    ...
    public List<String> getAgentsList() {
        ...
        try {
            ...
            resultSet = statement.executeQuery("SELECT " + AGENT_FIELD + " FROM " + AGENT_TABLE + " ORDER BY " + AGENT_FIELD);
    ...
    public void addAgent(String agent) {
        ...
        try {
            statement = getStatement();
            statement.executeUpdate("INSERT INTO " + AGENT_TABLE + " (ST_AGENT) VALUES ('" + agent + "')");
            ...
            resultSet = statementSelectCIs.executeQuery("SELECT I_ID FROM CI_T");
            String ci_id;
            while (resultSet.next()) {
                ci_id = String.valueOf(resultSet.getInt(1));
                try {
                    statement.executeUpdate("INSERT INTO " + RATIO_TABLE + " (I_AGENT_ID, I_CI_ID, I_RATIO) VALUES (" + agent_id + "," + ci_id + ",'0')");
                } catch (SQLException e) {
                }
            }
        } catch (SQLException e) {
    ...
    public List<String> getCIsList() {
        ...
        try {
            statement = getStatement();
            ResultSet rs = statement.executeQuery("SELECT ST_CI FROM " + CI_TABLE + " ORDER BY " + CI_FIELD);
            while (rs.next()) {
    ...
    public void addCI(String ci) {
        ...
        try {
            statement = getStatement();
            statement.executeUpdate("INSERT INTO " + CI_TABLE + " (ST_CI) VALUES ('" + ci + "')");
    ...
    public void delAgent(String agent) {
        ...
        try {
            ...
            resultSet = statementSelectCI.executeQuery("SELECT I_ID FROM AGENT_T WHERE ST_AGENT='" + agent + "'");
            int agent_id = 0;
            if (resultSet.next()) {
                agent_id = resultSet.getInt(1);
            }
            statement.executeUpdate("DELETE FROM " + AGENT_TABLE + " WHERE I_ID ='" + agent_id + "'");
            statement.executeUpdate("DELETE FROM " + RATIO_TABLE + " WHERE I_AGENT_ID =" + agent_id);
        } catch (SQLException e) {
        } finally {
            try {
                if (statement != null)
    ...
    public void delCI(String ci) {
        ...
        try {
            statement = getStatement();
            statementSelectCI = connection.createStatement();
            resultSet = statementSelectCI.executeQuery("SELECT I_ID FROM CI_T WHERE ST_CI='" + ci + "'");
            int ci_id = 0;
            if (resultSet.next()) {
                ci_id = resultSet.getInt(1);
            }
            statement.executeUpdate("DELETE FROM " + CI_TABLE + " WHERE I_ID =" + ci_id + "");
            statement.executeUpdate("DELETE FROM " + RATIO_TABLE + " WHERE I_CI_ID =" + ci_id);
    ...
    public void updateRatio(String agent, String ci, Integer ratio) {
        ...
        try {
            ...
            agentSelectResult = agentSelectStatement.executeQuery("SELECT I_ID FROM " + AGENT_TABLE + " WHERE " + AGENT_FIELD + "='" + agent + "'");
            ...
            ciSelectResult = ciSelectStatement.executeQuery("SELECT I_ID FROM " + CI_TABLE + " WHERE " + CI_FIELD + "='" + ci + "'");
            if (!ciSelectResult.next())
                return;
            String ci_id = ciSelectResult.getString("I_ID");

            insertStatement = connection.createStatement();
            insertStatement.executeUpdate("UPDATE " + RATIO_TABLE + " SET I_RATIO = " + ratio + " WHERE I_AGENT_ID='" + agent_id + "' AND I_CI_ID= '" + ci_id + "'");
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
            try {
    ...
    public Map<String, Integer> getAgentRatios(String ci) {
        ...
        try {
            ...
            ResultSet rs = statement.executeQuery(
                    "SELECT " + AGENT_FIELD + " , " + RATIO_FIELD +
                            " FROM " + RATIO_TABLE + " r," + AGENT_TABLE + " a, " + CI_TABLE + " c " +
                            " WHERE a.I_ID=r.I_AGENT_ID " +
                            " AND c.I_ID=r.I_CI_ID " +
                            " AND c." + CI_FIELD + "='" + ci + "' " +
                            " ORDER BY " + AGENT_FIELD);
            while (rs.next()) {
                result.put(rs.getString(AGENT_FIELD), rs.getInt(RATIO_FIELD));
            }
            statement.close();
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public int getSumAgent(String agent) {
        ...
        try {
            ...
            resultSet = statement.executeQuery("SELECT SUM(r.I_RATIO) " +
                    ...
                    " WHERE r.I_AGENT_ID = a.I_ID AND a.ST_AGENT = '" + agent.toUpperCase() + "'");
            if (resultSet.next()) {
